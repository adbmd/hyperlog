class AnalysisEndpoints::ReceiveAnalysisController < ActionController::Base
  ...
  def receive_initial_analysis
    ...
    g = User.find(params[:user_id]).profile.github
    ...
    data[:repos].each_value do |repo_info|
      Repo.where(provider: 'github', provider_repo_id: repo_info['repo_id']).first_or_create do |repo|
  ...
  def add_tech_analysis_by_repo
    ...
    profile.tech_analysis['repos'][repo_name] = {
      libs: data['libs'],
      tech: data['tech'],
      tags: data['tags']
    }
