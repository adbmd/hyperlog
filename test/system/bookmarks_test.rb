...
class BookmarksTest < ApplicationSystemTestCase
  ...
  test "creating a Bookmark" do
    ...
    fill_in "Folder", with: @bookmark.folder
    check "Is hidden" if @bookmark.is_hidden
    check "Is pinned" if @bookmark.is_pinned
    fill_in "Profile", with: @bookmark.profile_id
    fill_in "Title", with: @bookmark.title
    check "To read" if @bookmark.to_read
